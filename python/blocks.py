...
def create_spn(shared_key, num_box, box_inlen, num_rounds, mixing_layer):
    ...
    for r in range(num_rounds):
        ...
        for xindex in range(input_len):
            ...
            if mixing_layer == 'xor':
                enc_ckt.add_gate_wids(gfun.XOR, [x[xindex], k[xindex]], out)
            elif mixing_layer == 'and':
                enc_ckt.add_gate_wids(gfun.AND, [x[xindex], k[xindex]], out)
            elif mixing_layer == 'or':
                enc_ckt.add_gate_wids(gfun.OR, [x[xindex], k[xindex]], out)
            elif mixing_layer == 'mux':
                secindex = random.randint(0, input_len-1)
                enc_ckt.add_gate_wids(gfun.MUX, [k[xindex], x[xindex], x[secindex]], out)
            else:
