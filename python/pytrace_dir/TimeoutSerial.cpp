...
{
	/*int block_size = 32;
	size_t num_blocks = s.size() / block_size;
	for (size_t bn = 0; bn <= num_blocks; bn++) {
		string sub = s.substr(bn * block_size, block_size);
		std::cout << "sedning " << sub << "\n";
		asio::write(port,asio::buffer(sub.c_str(),sub.size()));
	}*/
