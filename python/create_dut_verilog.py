...
if __name__ == '__main__':
    if len(sys.argv) != 6:
        print('usage: create_dut_verilog.py <enc_cir_bench> <dut_verilog_file> <top_verilog_file_pre> <top_verilog_file_post> <num_instance>')
        exit(1)
    ...
    with open(sys.argv[3], 'r') as fn:
        for ln in fn:
            if '#' in ln:
                ln = ln.replace('#NUM_INS', '{0}'.format(cir.num_allins()) )
                ln = ln.replace('#NUM_COPY', '{0}'.format(sys.argv[5]))
