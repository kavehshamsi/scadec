...
def lock_random_xor(cir, num_keys, key=[]):

    ecir = copy.deepcopy(cir)

    wire_ids = list(ecir.wires())
    locked_nets = random.sample(wire_ids, num_keys)
    
    if len(key) != num_keys:
        key = [bool(random.getrandbits(1)) for i in range(num_keys)]
        
    #print(key)
    
    assert(len(locked_nets) == len(key))
        
    for i, net in enumerate(locked_nets):
        if key[i] == 0:
            gid = insert_gate(ecir, gfun.XOR, net, 'enc{0}'.format(i))
        else:
            gid = insert_gate(ecir, gfun.XNOR, net, 'enc{0}'.format(i))
        kid = ecir.add_key_wire()
        ecir.gp.add_edge(kid, gid)
...
def add_key_mux(cir, nid, other_muxins):
    ...
    while len(cur_layer) > 1:
        ...
        if len(cur_layer) == 0 and len(next_layer) == 0:
            muxout = right_wid
            cir.add_gate_wids(gfun.MUX, [layer_keys[layer_count], muxin0, muxin1], muxout)
            break
...
def insert_gate(cir, gatefun, wid, wname='opened'):
    ...
    return cir.add_gate_wids(gatefun, inputs, right_wid)
