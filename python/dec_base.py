...
class CirDecrypt:
    ...
    def check_key(self, keyvec):
        ...
        eq = check_equivalence(cir1, cir2)
        if not eq:
...
class CirDecryptZ3(CirDecrypt):
    ...
    def build_iocir(self, enc_cir):
        ...
        for oid in copy.copy(self.iocir.outputs()):
            ...
            io_outs.append(noid)
            self.iocir.add_gate_wids(gfun.XNOR, {ioin, oid}, noid)
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print('usage: dec_base.py <sim_cir> <enc_cir> <iterations>')
        exit(1)
    sim_cir = Circuit(sys.argv[1])
    enc_cir = Circuit(sys.argv[2])

    cdec = CirDecryptZ3(sim_cir, enc_cir, int(sys.argv[3]))
    cdec.solve_exact()
